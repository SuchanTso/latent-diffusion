...
def load_mask(mask_dir , block_type , block_name , mask_id = 1 , mask_time = 21):
    mask_path = os.path.join(mask_dir , f"{block_name}_{block_type}" , f"mask_{mask_time}")
    print(f"loading mask_{mask_id} in {mask_path}")
    masks_norm_list = []
    for _, dirs, files in os.walk(mask_path):
        for i , file in enumerate(files):
            ...
            masks_norm  = (masks == mask_id).type(torch.float32)
            masks_norm_list.append(masks_norm)
            # print(f"loading mask index = {i} , files:{file_path}")
        break
    print(f"done loading {len(masks_norm_list)}")
    return torch.stack(masks_norm_list , dim=0)

def load_unique_mask(mask_dir , block_type , block_name , mask_id = 1 , mask_time = 21):
    masks = load_mask(mask_dir , block_type , block_name , mask_id , mask_time)
    for mask in masks:
        if mask.max() > 0:
            return mask
    assert False , f"mask_id = {mask_id} not found in {mask_dir} , block_type = {block_type} , block_name = {block_name} , mask_time = {mask_time}"
...
def main():
    ...
    def extract_specific_block_features(blocks ,block_num , feature_type="self_attn_q"):
        #remain for now since i will fix the whole pipline for feature extraction
        block_idx = 0
    ...
    mask_q = load_unique_mask(exp_config.config.mask_dir , "self_attn_q" , block_str , mask_id = 1)
    ...
    editted_img , _= ddim_process_sampling(model , sampler_ , z_T ,
                                        ddim_steps ,
                                        gen_prompt(model , exp_config.config.prompt), 
    ...
    save_samples(os.path.join(exp_config.config.save_dir , "prompt.png") , prompted_img)
    save_samples(os.path.join(exp_config.config.save_dir , "ddim.png") , no_valid_sample)
